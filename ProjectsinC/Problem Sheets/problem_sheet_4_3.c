...
/*.
3. Write a C program that declares three strings as character arrays each of some
maximum size. Input a string into the first character array. Input another string
into the second character array. In your program, compare the two strings and
say whether or not they are equal, and state the length of each string.
Concatenate the first string with the second string. Copy the concatenated string
into the third string and print it out. Caution: make sure the array size will
accommodate the concatenation process for the strings that you input.
*/
